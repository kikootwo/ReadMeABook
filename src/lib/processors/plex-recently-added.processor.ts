/**
 * Component: Plex Recently Added Check Processor
 * Documentation: documentation/backend/services/scheduler.md
 *
 * Lightweight polling for new Plex library items
 */
...
export async function processPlexRecentlyAddedCheck(payload: PlexRecentlyAddedPayload): Promise<any> {
  ...
  const logger = jobId ? createJobLogger(jobId, 'PlexRecentlyAdded') : null;
  ...
  try {
      ...
      if (!existing) {
        ...
        await prisma.plexLibrary.update({
          ...
          data: {
            ...
            userRating: item.userRating !== undefined ? item.userRating : existing.userRating,
        ...
        try {
          ...
          if (match) {
            ...
            await prisma.audiobook.update({
              ...
              data: { plexGuid: match.plexGuid, updatedAt: new Date() },
            });
    ...
    return {
      success: true,
      message: 'Recently added check completed',
      newCount,
      updatedCount,
      matchedDownloads,
    };
