...
- **Avatar cache (`User.discordAvatarUrl`):** admin surfaces showed a generic placeholder for Discord requesters, because a local account has no Plex/OIDC `avatarUrl` and a Discord CDN URL (`/avatars/{userId}/{hash}`) can't be derived from the stored user id. [discord-avatar.ts](../../src/lib/services/discord/discord-avatar.ts) `captureDiscordAvatar(user)` takes the URL off the interaction already being handled — called fire-and-forget from the **interaction router**, the single point every command/button/select passes through, so it never delays the 3-second ack and self-heals when someone changes their avatar. Writes only when the URL actually changed. Users with no custom Discord avatar stay `null` so the existing placeholder applies rather than pinning Discord's default image. Kept in its own column so a Plex/OIDC avatar is never overwritten; consumers prefer `discordAvatarUrl`, then `avatarUrl`, then the placeholder. Populates on the next interaction — **it does not backfill**.

## Commands
- **/request `<type>` `[query]`** (audiobook|ebook): search modal → `audibleService.search` → result dropdown → confirm (cover thumbnail) → create request → post live **request card** (see below). The optional `query` skips the modal and goes straight to results.
  ...
  - **E-book type is only offered when it can succeed.** `buildCommandDefinitions(deletePermission, ebookEnabled)` omits the E-book choice entirely when no e-book source is enabled, using the shared `isEbookRequestingEnabled()` from the ebook creator service so the offered types and the request path cannot drift. The request path still guards independently, for clients holding a stale command list.
  - **Sidecar rule surfaced up front.** The Web UI hides the constraint by only showing "Fetch Ebook" on owned titles, but Discord searches the whole Audible catalogue, so the e-book search modal's placeholder reads *"Must already be in your library as an audiobook"* and both result dropdowns (modal and inline-`query`) append the same note via `searchResultsBody`. It warns rather than filters: an unowned pick still fails at confirm.
  - **Result rows are media-aware:** `buildSearchSelect` omits "Narrated by" for e-book searches (narrator is audiobook-only, matching the cards and embeds).
...
- Click authority: RMAB admin **or** holds `adminRoleId`. Approve/Deny → shared `processRequestApproval` ([request-approval.service.ts](../../src/lib/services/request-approval.service.ts), also used by the Web approve route; the transition is claimed atomically so concurrent clicks can't double-process — see [request-approval.md](../admin-features/request-approval.md)).
- **The Discord rewrite lives in the shared service, not the button handler.** `syncDiscordOnDecision` runs on every successful decision from **any** surface — Discord buttons, the Web UI Deny button, or an API token — and: rewrites the approval message to **✅ Request Approved** / **🚫 Request Denied** with its buttons removed (`applyDecisionToApprovalMessage`), refreshes the requester's request card (`editRequestCards`), and DMs the requester the outcome (`notifyRequesterOfDecision`). Gated on a running bot, dynamically imported, never throws.
  - Actor attribution: the deciding user's linked `discordUserId`, or the raw id for an admin-role holder with no RMAB account. When neither exists (a Web UI admin with no linked Discord account) the "by" field is **omitted** rather than rendering `<@null>`.
  - The button handler deliberately performs no message edit, card refresh, or DM of its own, so the two surfaces cannot drift apart again.
- **Stale clicks reconcile.** When a click loses a race (already decided, or cancelled elsewhere), `reconcileApprovalMessage` re-renders the message from the request's *current* persisted state — denied → 🚫 Denied, soft-deleted → 🚫 Cancelled, anything past `awaiting_approval` → ✅ Approved — instead of leaving it reading "⏳ Pending" with merely greyed-out buttons.
- **Two independent approval surfaces.** The bot's interactive approval message (above, in `admin_notify_channel_id`/`request_channel_id`) is separate from the **Discord webhook notifications** (Settings → Notifications → Discord, [notifications.md](../backend/services/notifications.md)), which also emit a `request_pending_approval`/`request_approved` embed. They serve different setups (the webhook works without the bot; only the bot's message has buttons) and normally reach different audiences (channel announcement vs the requester's decision DM). **Recommendation:** point the webhook notification channel and the bot approval channel at **different channels** — pointing both at the same channel intentionally produces two messages (one actionable, one informational), not a bug. Suppressing one is avoided because it would regress non-bot/email setups or drop the requester's personal DM.

## Modules (src/lib/services/discord/)
- `discord-bot.service.ts` — singleton client; `start/stop/restart/isReady/refreshCommands`; registers commands on `ready`; routes interactions.
- `interaction-router.ts` — dispatch by command name / decoded customId; defers within 3s.
- `command-definitions.ts` — SlashCommandBuilder defs (guild-scoped JSON); takes `ebookEnabled` so the E-book request type is only offered when a source is configured.
...
- `discord-cards.ts` — post/update the live request card (public + DM) and the approval message: `postRequestCards`, `editRequestCards`, `cancelApprovalMessage`, `applyDecisionToApprovalMessage`, `notifyRequesterOfDecision`, `reconcileApprovalMessage`.
- `discord-avatar.ts` — caches the requester's Discord CDN avatar onto `User.discordAvatarUrl` from each interaction.
...
The card/approval helpers log **why they skipped**, not just failures — `bot not running`, `no cards recorded`, `no approval message recorded`, `requester has no linked Discord account` — plus a line on each successful edit. These paths are all best-effort and silent by design, so without the skip logs a sync that never fired was indistinguishable from one that worked. (A `no cards recorded` line right after a `request_pending_approval` notification is expected: the notification hook fires before the card has been posted and recorded.)
...
- **Concurrency:** the client is only assigned after `login()` resolves, so `restart()` first drains any in-flight start (`starting` holds the in-flight promise; a concurrent `start()` joins it rather than returning early). A `generation` counter, bumped by `stop()`, makes a start whose epoch moved destroy its own connection instead of publishing a client built from stale config — and suppresses its `ready`. Without both, an admin save could silently fail to apply.
- **Singleton:** stashed on `globalThis` (same pattern as [db.ts](../../src/lib/db.ts)) so Next dev-mode HMR can't spawn a second gateway connection that handles every interaction twice.
- **Settings save:** all `discord.*` config writes (including the encrypted token) run in one `prisma.$transaction`, so a partial save can't leave the bot reloading a half-written config. The cache clear and `restart()` run only after it commits.
- **Command refresh without a restart:** `refreshCommands()` re-registers the guild commands against the live client. `PUT /api/admin/settings/ebook` calls it (best-effort, gated on a running bot, dynamically imported) because enabling/disabling e-book sources changes whether `/request` offers the E-book type. Without it the choice would only appear or disappear after an unrelated Discord settings save restarted the bot.

## Edge cases

Bot disabled/unconfigured → init skips start (and skips loading discord.js). Unlinked user → ephemeral guidance. Invalid token → caught at start + surfaced by Test. Gateway auto-reconnects; commands re-registered on each `ready`. Stale/double approval → status guard + buttons disabled. Approving a cancelled request → blocked by the `deletedAt` guard, and deletion from any surface now rewrites the card and strips the approval buttons (see [request-deletion.md](../admin-features/request-deletion.md)). DM closed → logged, non-fatal.
